...
4. `anglerfish-core` depends on `esca` and `log`, and on no chess library of
   its own.
...
command, `Position` inside the search. What changes immediately:

| Item | Shape |
|---|---|
| `Evaluator` trait | `fn value(&self, pos: &Position, facts: &Facts) -> Score` and `fn batch(&self, positions: &[(Position, Facts)], out: &mut [Score])`. A batching entry point exists from day one because an MCTS-style search needs it and an alpha-beta search may ignore it. |
...
| **Stability** | Golden fixtures: 1000 fixed FENs and their vectors under `rs_anglerfish/esca/tests/data/`, regenerated by `cargo run --release --example fixtures`, stored per schema version. A changed output fails until the group version is bumped and a new fixture added. Old fixtures are kept and still checked while their group version is supported. |
