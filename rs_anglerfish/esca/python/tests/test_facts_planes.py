...
#: that d3 attacks.
PAWN_DUEL = "4k3/1p3p2/2p5/1P6/6P1/3p4/2P5/4K3 b - - 0 1"

#: The c3 knight and the d6 bishop are each attacked with nothing defending them.
LOOSE_PIECES = "4k3/1p6/2nb4/8/1p6/2N5/P7/3RK3 w - - 0 1"

#: The bishops on b4 and b5 pin the knight in front of each king; a pinned
#: knight still attacks everything a free one would.
CROSS_PINS = "4k3/8/2n5/1B6/1b6/2N5/8/4K3 w - - 0 1"

#: A pin along a file against a pin along a rank; both pinned knights hang.
FILE_PINS = "4k3/7p/4n3/4R3/8/8/6P1/r1N1K3 w - - 0 1"

#: Black to move, a pin and a loose pawn a side, and every plane occupied.
COUNTERPLAY = "r5k1/6p1/2n4p/3p4/1b1P4/1BN3P1/PP6/4K2R b K - 0 1"

#: The rook's file ends at the king it checks: d6 and d7 are not attacked.
THROUGH_THE_KING = "8/8/8/3k4/8/8/P7/3RK3 b - - 0 1"

#: A Chess960 middlegame, each king tucked behind its own rook; a knight of each
#: side stands where nothing defends it.
TUCKED_KINGS = "1kr5/pp3p2/4N3/6n1/2n5/3B4/PP6/1KR5 w Cc - 0 1"
...
CROSSED_BISHOPS = "3r2k1/6pp/6n1/8/2bN4/8/BP4P1/3K3R w Hd - 0 1"
...
LOOSE_QUEEN = "1r5k/6pp/8/2b5/3Q4/1P6/P1P5/1K6 w - - 0 1"
...
def planes_layout() -> dict[str, slice]:
    """Where each `planes` feature sits inside its group, per the schema's own text."""
...
#: The offset and width of every `planes` feature.
PLANES = planes_layout()


def square_at(index: int) -> str:
    """The square a plane index names, a1 = 0 and h8 = 63."""
    return f"{'abcdefgh'[index % 8]}{index // 8 + 1}"


def plane(fen: str, feature: str) -> set[str]:
    """The squares set in the plane `feature`, read out of the row `fen` encodes
    to at the offset and width the schema gives the feature, and named in the
    mover's view."""
    row = esca.encode([fen], groups=["planes"])[0]
    bits = row[PLANES[feature]]
    assert all(value in (0.0, 1.0) for value in bits), "a plane holds one bit a square"
    return {square_at(index) for index, value in enumerate(bits) if value == 1.0}
...
def test_an_attack_map_holds_every_square_its_side_could_capture_on(
    fen: str, us: str, them: str, facts_of: FactsOf, squares: Squares
) -> None:
    planes = facts_of(fen).planes
    assert set(planes.attacked[esca.US]) == squares(us)
    assert set(planes.attacked[esca.THEM]) == squares(them)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (PAWN_DUEL, "a6 b5 c2 c6 d5 e2 e6 g6", "a6 b3 c6 d3 f5 h5"),
        (LOOSE_PIECES, "b3", "a3 a6 c3 c6"),
        (CROSS_PINS, "", ""),
        (FILE_PINS, "f3 h3", "g6"),
        (COUNTERPLAY, "c4 e4 f6 g5 h6", "a3 b3 c3 c5 e5 f4 h4"),
        (THROUGH_THE_KING, "", "b3"),
    ],
    ids=[
        "start",
...
def test_a_pawn_attack_map_holds_the_diagonals_and_nothing_a_pawn_pushes_onto(
    fen: str, us: str, them: str, facts_of: FactsOf, squares: Squares
) -> None:
    planes = facts_of(fen).planes
    assert set(planes.attacked_by_pawns[esca.US]) == squares(us)
    assert set(planes.attacked_by_pawns[esca.THEM]) == squares(them)
...
def test_a_hanging_unit_is_attacked_by_the_opponent_and_defended_by_nobody(
    fen: str, us: str, them: str, facts_of: FactsOf, squares: Squares
) -> None:
    planes = facts_of(fen).planes
    assert set(planes.hanging[esca.US]) == squares(us)
    assert set(planes.hanging[esca.THEM]) == squares(them)
...
def test_an_absolutely_pinned_unit_is_the_one_thing_between_a_slider_and_its_own_king(
    fen: str, us: str, them: str, facts_of: FactsOf, squares: Squares
) -> None:
    planes = facts_of(fen).planes
    assert set(planes.pinned[esca.US]) == squares(us)
    assert set(planes.pinned[esca.THEM]) == squares(them)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("attacked_by_our_pawns", "f3 h3"),
        ("attacked_by_their_pawns", "g6"),
        ("our_hanging", "c1"),
        ("their_hanging", "e6"),
        ("our_pinned", "c1"),
        ("their_pinned", "e6"),
    ...
    ids=[
        "attacked_by_us",
        "attacked_by_them",
        "attacked_by_our_pawns",
        "attacked_by_their_pawns",
        "our_hanging",
        "their_hanging",
        "our_pinned",
        "their_pinned",
...
def test_white_to_move_writes_each_plane_as_the_board_stands(feature: str, expected: str, squares: Squares) -> None:
    assert plane(FILE_PINS, feature) == squares(expected)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("attacked_by_our_pawns", "c5 e5 f3 g4 h3"),
        ("attacked_by_their_pawns", "a6 b6 c4 c6 e4 f5 h5"),
        ("our_hanging", "d4"),
        ("their_hanging", "d5"),
        ("our_pinned", "d4"),
        ("their_pinned", "c6"),
...
def test_black_to_move_writes_each_plane_with_the_ranks_turned_around(
    feature: str, expected: str, squares: Squares
) -> None:
    """The mover's view flips rank *r* onto rank 9-*r* and leaves the files
    alone, so every plane of a Black-to-move position is written upside down."""
    assert plane(COUNTERPLAY, feature) == squares(expected)
...
def test_the_planes_of_a_chess960_position_read_the_placement_as_they_find_it(
    fen: str,
    attacked: tuple[str, str],
    ...
    """No plane reads the back rank as a starting square or the castling rights,
    so a Chess960 placement answers exactly as any other placement would."""
    planes = facts_of(fen, esca.CHESS960).planes
    for side in (esca.US, esca.THEM):
        assert set(planes.attacked[side]) == squares(attacked[side])
